...
export function buildProof({ markets, sets, generated, prevHistory = [] }) {
  ...
  const fakeArbs = sets
    .filter((s) => s.buy_set_gross !== null && s.buy_set_gross < 1 && s.verdict !== "CANDIDATE")
    .sort((a, b) => a.buy_set_gross - b.buy_set_gross)
    .map((s) => ({
      slug: s.slug,
      title: s.title,
      url: s.url,
  ...
  const summary = {
    ...
    share_5pp_edge_untradeable: share(hurdles, (h) => h > 0.05),
  ...
  const lite = {
    ...
    eaten3: summary.share_3pp_edge_untradeable,
  ...
  return {
    ...
    definitions: {
      ...
      share_3pp_edge_untradeable: "Share of liquid markets where a forecast 3pp away from the mid still has no +EV trade at 1 share.",
